...
public class DefFilter
{
    ...
    {
        if (!CanAcceptDefPath(def.DefName))
            return false;

        if(FilterDef.RequiredCompTypes?.Any() ?? false)
        {
            foreach (var req in FilterDef.RequiredCompTypes)
                if (!def.CompDefs.Any(x => x.CompInstClassName == req))
                    return false;
        }
        return true;
    }
